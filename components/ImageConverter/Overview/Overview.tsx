import overviewData from '@/data/tools/image-converter/overview.json';

export default function Overview() {
  return (
    <div className="bg-gray-50 dark:bg-gray-800 border-t border-gray-200 dark:border-gray-700">
      <div className="max-w-7xl mx-auto px-4 py-16 sm:px-4 lg:px-8">
        <div className="max-w-3xl">
          <h2 className="text-3xl font-bold text-gray-900 dark:text-gray-100 mb-4">{overviewData.title}</h2>
          <p className="text-xl text-gray-600 dark:text-gray-300 mb-6">{overviewData.subtitle}</p>
          <p className="text-lg text-gray-700 dark:text-gray-300 mb-8">{overviewData.description}</p>

          {overviewData.backendRequired && (
            <div className="mb-8 p-4 bg-blue-50 dark:bg-blue-900/20 border border-blue-200 dark:border-blue-800 rounded-lg">
              <p className="text-sm font-semibold text-blue-800 dark:text-blue-300 mb-2">Backend Service Required</p>
              <p className="text-sm text-blue-700 dark:text-blue-400">{overviewData.backendNote}</p>
            </div>
          )}

          <div className="mb-8">
            <h3 className="text-2xl font-semibold text-gray-900 dark:text-gray-100 mb-4">Features</h3>
            <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
              {overviewData.features.map((feature, idx) => (
                <div key={idx} className="p-4 bg-white dark:bg-gray-700 rounded-lg border border-gray-200 dark:border-gray-600">
                  <h4 className="font-semibold text-gray-900 dark:text-gray-100 mb-2">{feature.title}</h4>
                  <p className="text-sm text-gray-600 dark:text-gray-300">{feature.description}</p>
                </div>
              ))}
            </div>
          </div>

          <div className="mb-8">
            <h3 className="text-2xl font-semibold text-gray-900 dark:text-gray-100 mb-4">Use Cases</h3>
            <ul className="list-disc list-inside space-y-2 text-gray-700 dark:text-gray-300">
              {overviewData.useCases.map((useCase, idx) => (
                <li key={idx}>{useCase}</li>
              ))}
            </ul>
          </div>

          {overviewData.tips && overviewData.tips.length > 0 && (
            <div>
              <h3 className="text-2xl font-semibold text-gray-900 dark:text-gray-100 mb-4">Tips</h3>
              <ul className="list-disc list-inside space-y-2 text-gray-700 dark:text-gray-300">
                {overviewData.tips.map((tip, idx) => (
                  <li key={idx}>{tip}</li>
                ))}
              </ul>
            </div>
          )}
        </div>
      </div>
    </div>
  );
}
