...
const BLOCKLIST = process.env.HARVEST_BLOCKLIST
  ...
  : /flower|fitness|insurance|halloween|incontinen|casino|\bbet\b|dating|pharma/i;
...
(async () => {
  ...
  while (offset < total && (!LIMIT || offers.length < LIMIT)) {
    const res = await apiGet(`${API_BASE}/all_partners_programs/?limit=${pageSize}&offset=${offset}${CATEGORIES ? `&categories=${encodeURIComponent(CATEGORIES)}` : ''}`);
    if (res.status !== 200) {
  ...
  targets = targets.filter((o) => !BLOCKLIST.test(o.advcampaign?.name || ''));
